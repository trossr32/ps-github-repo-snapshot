...
public class GithubService
{
    ...
    /// </summary>
    /// <returns></returns>
    public static async Task<List<GithubRepo>> GetRepositories(FilterConfig filter)
    {
    ...
    /// <returns></returns>
    public static async Task DownloadAndExtractRepository(GithubRepo repo, string path, string branch, bool extract)
    {
        using var client = GetClient();
        ...
        if (extract)
            await ZipFile.ExtractToDirectoryAsync(zip, repoPath);
    }
